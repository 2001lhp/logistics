	...
	>
		<view class="card" v-for="item in taskList" :key="item.id">
			...
			<view class="footer">
				...
				<navigator v-if="item.status === 2" class="action" :url="`/subpkg_task/delivery/delivery?id=${item.id}`">交付</navigator>
				<navigator v-if="item.status === 4" class="action" :url="`/subpkg_task/record/record?id=${item.transportTaskId}&time=${item.actualDepartureTime}`">回车登记</navigator>
			</view>
		</view>
